"""Runs a function and then a few assertions that should fail"""
...
from hyalus.config.tags import Medium, RegressionTest
...
STEPS = [
    ...
    AssertEQ(("output/food.json", ["best_cuisine"]), "American"),
    AssertEQ(("output/coffee.json", ["best_coffee_shop"]), "Ozo"),
]

TAGS = [Medium(info="Should take a bit"), RegressionTest("Test expected to fail until <issue> fixed")]
